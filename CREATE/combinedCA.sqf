...
if (_involveCSAT) then {
	...
	for "_i" from 1 to _maxCounter do {
		private _opforSpawn = false; //Sparker
		if (_i == _maxCounter) then {
			_vehicleType = selectRandom opHeliTrans; //Helis spawn blu bases under them
			_opforSpawn = true;
		} else {
			_vehicleType = selectRandom opAir;
			_opforSpawn = false; //Planes don't spawn blu bases under them
		};
		...
		_vehicle setVariable ["OPFORSpawn", _opforSpawn];
...
if !(_airport == "") then {
	...
	for "_i" from 1 to 3 do {
		...
		_vehicle = _vehicleData select 0;
		if(_vehicleType in planes) then
		{ //Planes shouldn't spawn stuff under them
			_vehicle setVariable ["OPFORSpawn",false];
		}
		else
		{ //Helicopters should spawn stuff under them
